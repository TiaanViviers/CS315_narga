...
class HMM(object):
    ...
    def _viterbi(signal, trans, dists):
        ...
        emitlog = np.array([[dists[j].logf(signal[:, t]) for t in range(n_obs)]
                            for j in range(n_states)])

        delta = np.full((n_states, n_obs), -np.inf)
        psi = np.full((n_states, n_obs), -1, dtype=int)

        delta[:, 0] = logtrans[-1, :n_states] + emitlog[:, 0]

        for t in range(1, n_obs):
            for j in range(n_states):
                prev = delta[:, t-1] + logtrans[:n_states, j]
                psi[j, t] = int(np.argmax(prev))
                delta[j, t] = prev[psi[j, t]] + emitlog[j, t]

        last_scores = delta[:, -1] + logtrans[:n_states, -1]
        last_state = int(np.argmax(last_scores))
        ll = float(last_scores[last_state])
    ...
    def _score(signal, seq, trans, dists):
        ...
        ll = np.log(trans[-1, seq[0]]) + dists[seq[0]].logf(signal[:, 0])
        for t in range(1, signal.shape[1]):
            ll += np.log(trans[seq[t-1], seq[t]]) + dists[seq[t]].logf(signal[:, t])
    ...
    def _forward(signal, trans, dists):
        ...
        alpha[:, 0] = logtrans[-1, :n_states] + emitlog[:, 0]

        for t in range(1, n_obs):
            for j in range(n_states):
                alpha[j, t] = np.logaddexp.reduce(alpha[:, t-1] + logtrans[:n_states, j]) + emitlog[j, t]

        return float(np.logaddexp.reduce(alpha[:, -1] + logtrans[:n_states, -1]))
    ...
    def _calcstates(self, trans, dists):
        ...
        n_states = len(dists)
        state_rows = []
        trans_counts = np.zeros_like(trans, dtype=float)
        ll = 0.0

        for signal in self.signals:
            seq, signalll = HMM._viterbi(signal, trans, dists)
            ll += signalll

            signal_states = np.zeros((signal.shape[1], n_states), dtype=bool)
            signal_states[np.arange(signal.shape[1]), seq] = True
            state_rows.append(signal_states)
        ...
        used = np.any(states, axis=0)
        states = states[:, used]

        keep_cols = np.hstack((used, [True]))
        keep_rows = np.hstack((used, [True]))
        trans_counts = trans_counts[keep_rows][:, keep_cols]

        row_sums = trans_counts.sum(axis=1, keepdims=True)
        trans = trans_counts / row_sums
        return states, trans, float(ll)
    ...
    def _updatecovs(self, states):
        ...
        for j in range(n_states):
            obs = self.data[:, states[:, j]]
            if obs.shape[1] == 0:
                cov = np.eye(d)
                mean = np.zeros(d)
            else:
                mean = np.mean(obs, axis=1)
                centered = obs - mean[:, np.newaxis]
                cov = centered.dot(centered.T) / obs.shape[1]
                if self.diagcov:
    ...
    def _em(self, trans, states):
        ...
        while not converged and iters <  self.maxiters:
            ...
            if oldLL == 0:
                relerr = abs(newLL - oldLL)
            else:
                relerr = abs((newLL - oldLL) / oldLL)
            converged = relerr < self.rtol
            oldLL = newLL
            iters += 1
    ...
    def sample(self):
        ...
        def draw_discrete_sample(discr_prob):
            ...
            if not np.isclose(np.sum(discr_prob), 1.0):
                raise ValueError('The sum of the discrete probabilities should add to 1')
            x = np.cumsum(discr_prob)
            r = np.random.rand()
            return int(np.searchsorted(x, r, side='right'))
        #######################################################################
        
        n_states = self._n_states()
        state = draw_discrete_sample(self.trans[-1])
        samples = []
        states = []

        while state != n_states:
            samples.append(self.dists[state].sample(1).flatten())
            states.append(state)
            state = draw_discrete_sample(self.trans[state])

        if len(samples) == 0:
            return np.empty((self.dists[0].get_mean().shape[0], 0)), np.array([], dtype=int)
        return np.column_stack(samples), np.array(states, dtype=int)
